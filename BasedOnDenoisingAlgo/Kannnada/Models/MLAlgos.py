...
#SVM
def SVectorMachine(X_train, Y_train, X_test, Y_test, kernelu = 'rbf', itr = -1, Cval = 1.0, deguree = 3, functionu = 'ovo'):
#     svc = SVC( kernel='rbf',max_iter=1000,decision_function_shape='ovo',random_state=10)
    svc = SVC( kernel= kernelu, degree=deguree, max_iter = itr, decision_function_shape= functionu, C = Cval, random_state=10)
...
def Dtree(X_train, Y_train, X_test, Y_test,deepth=None, split = 2):
    #Dtree = DecisionTreeClassifier(criterion='entropy', random_state=10)
    Dtree = DecisionTreeClassifier(max_depth=deepth, random_state=10, min_samples_split = split)
...
#RandomForest
def Rforest(X_train, Y_train, X_test, Y_test,deepth=None, split = 2):
